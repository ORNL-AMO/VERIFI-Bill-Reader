...
@app.get("/")
async def root():
    return {"message": "Welcome to the PDF to Excel API! Use the marker/upload endpoint to upload a PDF."}
    #return HTMLResponse(
     #   """
#<h1>Marker API</h1>
#<ul>
...
async def convert_pdf_upload(
    ...
    force_ocr: Optional[bool] = Form(default=True),
    ...
    #return results
    # Handle the output file based on the format
    ...
    with open(output_path, "w", encoding="utf-8") as output_file:
        output_file.write(results["output"])  # Save the converted text

    # Return the file path and success message
    return {
        "success": True,
        "message": f"File converted and saved as {output_filename}",
        "file_path": output_path,
        "output_format": output_format,
        # Optional: Include the output directly in the response for Angular
        "output": results["output"] if output_format in ["markdown", "json"] else None,
    }
