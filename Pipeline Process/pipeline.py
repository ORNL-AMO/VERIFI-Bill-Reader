...
# Function to extract a csv output for each section in the above section fields
def extract_section_data(section_name, fields, markdown_content, json_content, raw_pdf_content):
    ...
    # Fill in keywords with values for custom prompt
    ...
    # Hardcoded basic prompt below (Uncomment if txt for custom prompt is not being used)
    ''' 
    prompt = (
    ...
    # Output the raw AI response to a text file (Will only appear in output_dir folder if clean_output_dir is disabled). 
    ...
    # If the response is empty create a blank dataframe for the section (This should not happen unless model returns a bad result)
